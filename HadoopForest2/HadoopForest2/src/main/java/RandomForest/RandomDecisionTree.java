...
public class RandomDecisionTree extends Classifier {
    class TreeNode {
        int[] set;                // 样本下标集合
    ...
    private SplitData attribute_selection(TreeNode node) {
        ...
        int numAttributeToPick = 5;
        if (node.set.length < numAttributeToPick) return result;

        // 生成随机选取的属性
        numAttributeToPick = (int) (Math.log(1 + node.attr_index.length) / Math.log(2));
    ...
    private SplitData split_with_attribute(int[] set, int attribute) throws SplitException {
        ...
        if (_isCategory[attribute]) {
            ...
            for (Double key : index_recorder.keySet()) {
